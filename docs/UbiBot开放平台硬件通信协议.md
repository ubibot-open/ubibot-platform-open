...
| 来源 | 说明 |
| --- | --- |
| 手动添加 | 运维人员提前在设备管理中录入，平台生成 DeviceSecret，运维在生产/烧录环节把这个值写入设备。这类设备第一次连接平台即可直接完成 §4 的激活流程 |
| 自主激活 | 设备出厂时已经烧录好自己的 DeviceSecret，但平台数据库里还没有对应记录（例如新采购了一批设备、或补录之前漏掉的设备）。这类设备第一次调用 /auth/activate 时 sn 在平台上找不到，平台会自动新建一条状态为"待确认"的设备记录，但**不会**为它生成密钥——因为没有任何办法把之后生成的密钥再写回一台已经出厂的设备 |

自主激活设备在完成密钥绑定之前，对 §4/§5/§7.1 的所有认证请求一律返回与"设备不存在/已被禁用"相同的错误（1103），不做区分。设备本身感知不到任何差异，只需按既有的重试逻辑不断重试激活，直到密钥绑定完成为止。

**密钥绑定由专用工具/App完成，而不是设备自己上报**

自主激活设备的真实密钥只存在于硬件里（例如通过串口指令或蓝牙可以读取），需要一个专用的现场工具或App来完成"读取设备密钥 → 提交给平台"这个一次性绑定动作：

1. 工具/App 通过串口或蓝牙连接设备，读取其 DeviceSecret。
2. 工具/App 调用 `GET /api/v1/auth/public-key` 获取平台的 RSA 公钥（PKCS#1 PEM格式，2048位）。
3. 工具/App 用该公钥对 DeviceSecret 做 RSA-OAEP（SHA-256）加密，并做 Base64 编码。
4. 工具/App 调用 `POST /api/v1/auth/bind-key`，提交设备序列号与加密后的密钥。

**获取平台公钥：GET /api/v1/auth/public-key**

响应：

```json
{ "c": 0, "public_key_pem": "-----BEGIN RSA PUBLIC KEY-----\n...\n-----END RSA PUBLIC KEY-----\n" }
```

**提交密钥绑定：POST /api/v1/auth/bind-key**
...
{
  "sn": "sn_ws1_20001_1",
  "secret": "<Base64( RSA-OAEP-SHA256( 平台公钥, DeviceSecret ) )>"
}
```
...
| --- | --- |
| sn | 目标设备序列号 |
| secret | DeviceSecret 原文用平台公钥做 RSA-OAEP-SHA256 加密后的密文，Base64编码——密钥原文不会以明文形式出现在这个请求里 |

平台收到后用自己的私钥解密还原出真实的 DeviceSecret，与该 sn 对应的自主激活设备记录绑定保存；如果这台设备此前处于"待确认"状态，绑定成功后自动转为已启用，此后即可按 §4 正常走激活流程——设备自己签名时用的密钥与平台保存的密钥此时是一致的，能够通过校验。

错误响应（sn不存在、该设备不是自主激活来源、或密文无法解密，统一归为一类，不向调用方区分具体原因）：

```json
{ "c": 1104, "m": "device not found or not eligible for key binding" }
```

**后台管理员手动设置密钥**

如果现场不具备运行上述工具/App的条件，管理员也可以直接在设备管理页面里为自主激活设备手动填入其真实密钥——效果与上面的加密提交接口等价（同样会在设备处于"待确认"状态时自动转为已启用），可作为人工兜底通道。
...
| 400 | 1104 | 密钥绑定失败（§4.1）：sn不存在/不是自主激活设备/密文无法解密 | 由现场工具核实设备序列号与真实密钥后重试，与设备自身的激活重试逻辑无关 |
